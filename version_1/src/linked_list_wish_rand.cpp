...
void LinkedListWithRand::deserialize(const std::string &filename)
{
    ...
    while (getline(file, line))
    {
        ...
        resolver_.registerNode(tail_, rand_index);
    }

    if (resolver_.getPendingCount() > 0)
    {
        std::cerr << "Warning: " << resolver_.getPendingCount()
...
void LinkedListWithRand::serialize(const std::string &filename) const
{
    ...
    while (current)
    {
        ...
        if (current->rand)
        {
            rand_index = resolver_.getIndex(current->rand);
        }
